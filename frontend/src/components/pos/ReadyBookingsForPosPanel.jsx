...
export default function ReadyBookingsForPosPanel({ onSelectBooking }) {
  ...
  async function loadBookings() {
    try {
      ...
      console.error("ready bookings error", error?.response?.data || error);
      const detail =
  ...
  async function markPaidManually(booking) {
    ...
    try {
      ...
      toast.error(error?.response?.data?.detail || "تعذر تعليم الحجز كمدفوع");
    }
  }
