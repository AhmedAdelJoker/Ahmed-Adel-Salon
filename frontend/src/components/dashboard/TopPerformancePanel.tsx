...
import { Activity, Receipt, Scissors, Sparkles } from "lucide-react";
...
export interface TopPerformanceWidgets {
  top_barber?: TopBarber | null;
  top_service?: TopService | null;
  latest_invoices?: LatestInvoice[];
  latest_activities?: LatestActivity[];
  [key: string]: unknown;
}

export default function TopPerformancePanel({ widgets = {}, className = "" }: { widgets?: TopPerformanceWidgets; className?: string }) {
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <div className="rounded-3xl border border-border bg-soft/50 p-4">
          ...
          <div className="text-2xl font-black text-accent">
            {currency(Number(topBarber?.total_revenue || 0))}
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-border bg-soft/50 p-4">
          ...
          <div className="text-2xl font-black text-accent">
            {currency(Number(topService?.total_revenue || 0))}
          </div>
        </div>
      </div>
              ...
              {latestInvoices.map((item) => (
                ...
                >
                  ...
                  <div className="shrink-0 text-sm font-black text-success">
                    {currency(Number(item.total_amount || 0))}
                  </div>
                </div>
              ))}
            </div>
