...
export const DEFAULT_TRUST_BADGES = ["خدمة آمنة", "5 نجوم", "تجربة عائلية"];
...
export const DEFAULT_LANDING_TESTIMONIALS = [
  ...
  },
  {
    ...
    role: "عميل جديد",
    quote:
      "أول زيارة وكانت ممتازة، الاستقبال محترم واللوك النهائي فعلاً فرق معي.",
  },
];
...
function normalizeList<T, D>(values: unknown, defaults: D[], mapper: (current: any, fallback: D, index: number) => T): T[] {
...
export function buildLandingSiteContent(settings: Record<string, any> = {}): { stats: any[]; features: any[]; trustBadges: any[]; testimonials: any[]; copy: Record<string, any> } {
  ...
  const copy: Record<string, any> = Object.keys(DEFAULT_LANDING_COPY).reduce((acc: Record<string, any>, key: string) => {
    acc[key] = settings?.[key] || (DEFAULT_LANDING_COPY as Record<string, any>)[key];
    return acc;
  }, {});
