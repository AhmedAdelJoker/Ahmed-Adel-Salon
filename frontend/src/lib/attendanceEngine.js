 ...
 */
export const calculateAdvancedHours = (records, shift = { start: "10:00", end: "18:00" }) => {
  let totalMinutes = 0;
  ...
  sortedRecords.forEach((rec, index) => {
    ...
    if (rec.status === "in") {
      ...
      const isFirstIn = !sortedRecords.slice(0, index).some(r => r.status === "in");
      if (isFirstIn) {
        ...
        if (currentTime > shiftStart) {
          lateMinutes += (currentTime - shiftStart) / (1000 * 60);
        }
      }
    } else if (rec.status === "out") {
      if (lastInTime) {
        totalMinutes += (currentTime - lastInTime) / (1000 * 60);
        ...
        if (currentTime > shiftEnd) {
          overtimeMinutes += (currentTime - shiftEnd) / (1000 * 60);
        }
      ...
      if (lastBreakStartTime) {
        const duration = (currentTime - lastBreakStartTime) / (1000 * 60);
 ...
 */
export const calculatePayroll = (employee, stats, daysWorked = 30) => {
  ...
  const checkoutPenalty = stats.hasMissingCheckout ? deductionRules.missingCheckout : 0;
 ...
 */
export const analyzeProductivity = (stats, daysInMonth = 22) => {
  let score = 0;
