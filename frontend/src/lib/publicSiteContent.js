...
function normalizeList(values, defaults, mapper) {
...
export function buildLandingSiteContent(settings = {}) {
  ...
  const copy = Object.keys(DEFAULT_LANDING_COPY).reduce((acc, key) => {
    acc[key] = settings?.[key] || DEFAULT_LANDING_COPY[key];
    return acc;
  }, {});
