...
function InvoiceArchivePage() {
  ...
  async function toggleExpand(month) {
    const key = month.key;
    if (expandedKey === key) {
      setExpandedKey(null);
      return;
  ...
  async function loadMoreInvoices(month) {
    const key = month.key;
  ...
  async function handleReopenMonth(yearMonth) {
    try {
  ...
  async function handleCloseMonth(yearMonth) {
    try {
                              ...
                              <div className="max-h-96 overflow-y-auto p-3">
                                ...
                                <div className="space-y-1.5">
                                  {monthData.items.map((inv) => (
                                    <MonthInvoiceRow key={inv.id} invoice={inv} />
                                  ))}
                                </div>
                                {monthData.total >
...
function MonthInvoiceRow({ invoice }: any) {
  const method =
