...
const PermissionsManagement = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setIsActionLoading(true);
      if (editingUser) {
        await api.put(`/owner/employees/${editingUser.id}`, formData);
        toast.success("تم تحديث بروتوكولات الموظف");
      } else {
        await api.post("/owner/employees", formData);
        toast.success("تم إنشاء الهوية الأمنية بنجاح");
      }
      setIsModalOpen(false);
      fetchUsers();
    } catch (err) {
      toast.error(err.response?.data?.detail || "حدث خطأ أثناء حفظ التعديلات");
    } finally {
      setIsActionLoading(false);
    }
  };
    ...
    <div className="space-y-8 pb-24 erp-page-container" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        ...
        <Button
          ...
          onClick={() => {
            setEditingUser(null);
            setFormData({ role: "BARBER", status: "ACTIVE", isActive: true });
            setIsModalOpen(true);
          }}
