  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [transRes, invRes] = await Promise.all([
        reportService.getCashTransactions({
          from_date: startDate,
          to_date: endDate,
        }),
        api.get("/invoices", {
  ...
  const operationalMetrics = useMemo(() => {
    ...
    invoices.forEach((inv) => {
      ...
      (inv.items || []).forEach((item) => {
        ...
        const currentS = serviceMap.get(sName) || {
          name: sName,
          count: 0,
          revenue: 0,
        };
        serviceMap.set(sName, {
          ...currentS,
          count: currentS.count + 1,
          revenue: currentS.revenue + Number(item.price || 0),
        });

        const bName = item.barber_name || "غير محدد";
        const currentB = barberMap.get(bName) || {
          name: bName,
          count: 0,
    ...
    return {
      ...
      totalServices: Array.from(serviceMap.values()).reduce(
        (acc, s) => acc + s.count,
        0,
      ),
