...
function sanitizeFinancialRules(input = {}) {
  ...
  return {
    ...merged,
    taxRate: Math.max(0, Number(merged.taxRate ?? DEFAULT_FINANCIAL_RULES.taxRate) || 0),
    ...
    maxInvoiceWithoutApproval: Math.max(
      0,
      Number(merged.maxInvoiceWithoutApproval ?? DEFAULT_FINANCIAL_RULES.maxInvoiceWithoutApproval) || 0
    ),
    autoLockShiftHours: Math.max(
      0,
      Number(merged.autoLockShiftHours ?? DEFAULT_FINANCIAL_RULES.autoLockShiftHours) || 0
    ),
    enabledPaymentMethods: {
...
export default function FinancialRules() {
  ...
  async function loadData({ background = false } = {}) {
    try {
      ...
      } else if (!invoicesRes.ok || !expensesRes.ok || !shiftsRes.ok || !auditRes.ok) {
        lastMessage =
  ...
  async function saveRules() {
    try {
      ...
      toast.error(
        "تم الاحتفاظ بالقيم محليًا للعمل بها أوفلاين مؤقتاً.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="space-y-10 pb-20 animate-fade-in" dir="rtl">
      ...
      <div className="relative overflow-hidden rounded-[32px] bg-zinc-950 text-white p-8 lg:p-10 shadow-2xl shadow-black/20 border border-white/5">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row gap-8 items-start lg:items-center justify-between">
          <div className="flex items-center gap-6 max-w-2xl">
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-3 backdrop-blur-md">
                ...
                <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-emerald-100">إدارة اقتصاديات المؤسسة</span>
              </div>
              <h1 className="text-3xl lg:text-4xl font-black mb-2 leading-tight">
                مركز <span className="bg-gradient-to-l from-white to-white/40 bg-clip-text text-transparent">القواعد المالية</span>
              </h1>
              <p className="text-sm text-white/50 leading-relaxed font-medium max-w-xl">
                هيكل متقدم للتحكم في الضرائب، سياسات الخصم، طرق الدفع، ومعايير فتح وإغلاق الورديات لضمان أقصى درجات الرقابة.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto shrink-0 mt-6 lg:mt-0">
            ...
            >
              <RefreshCw size={16} className={cn("ml-2", refreshing && "animate-spin")} /> مزامنة
            </Button>
            <Button
              onClick={exportRules}
              className="h-12 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold transition-all backdrop-blur-md"
      ...
      {syncStatus.lastMessage ? (
        <InlineNotice tone={syncStatus.rulesSource === "local" ? "warning" : "info"} className="rounded-2xl border-border/50 shadow-sm">
          {syncStatus.lastMessage}
        </InlineNotice>
      ) : null}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {activeTab === "overview" && <OverviewPanel rules={rules} metrics={metrics} />}
          {activeTab === "rules" && <RulesPanel rules={rules} updateRule={updateRule} />}
          {activeTab === "payments" && <PaymentsPanel rules={rules} updateRule={updateRule} updatePaymentMethod={updatePaymentMethod} metrics={metrics} />}
          {activeTab === "audit" && <AuditPanel rows={filteredAuditRows} searchTerm={searchTerm} setSearchTerm={setSearchTerm} />}
        </motion.div>
      </AnimatePresence>
    </div>
  );
...
function OverviewPanel({ rules, metrics }) {
  const checks = [
    ...
    { title: "تنبيهات الخصم المشبوه", value: rules.enableSuspiciousDiscountAlerts, icon: BellRing },
    { title: "سبب الخصم إلزامي", value: rules.requireDiscountReason, icon: BadgePercent },
    { title: "موافقة المدير للخصومات", value: rules.requireManagerApprovalAboveLimit, icon: ShieldAlert },
    { title: "البيع مرتبط بالوردية", value: rules.requireOpenShiftForInvoices, icon: Lock },
    { title: "قفل فواتير الوردية المغلقة", value: rules.lockInvoicesAfterShiftClose, icon: FileLock2 },
    { title: "اعتماد المصروفات", value: rules.enableExpenseApproval, icon: FileCheck2 },
    { title: "منع المخزون السالب", value: !rules.allowNegativeInventory, icon: AlertTriangle },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard title="إيرادات اليوم" value={formatCurrency(metrics.revenue)} icon={Banknote} gradient="from-emerald-500/10 to-emerald-500/5 text-emerald-600" />
        <MetricCard title="مصروفات اليوم" value={formatCurrency(metrics.expenseTotal)} icon={CreditCard} gradient="from-amber-500/10 to-amber-500/5 text-amber-600" />
        <MetricCard title="صافي اليوم" value={formatCurrency(metrics.net)} icon={TrendingUp} gradient={metrics.net >= 0 ? "from-blue-500/10 to-blue-500/5 text-blue-600" : "from-red-500/10 to-red-500/5 text-red-600"} />
        <MetricCard title="إجمالي الخصومات" value={formatCurrency(metrics.discounts)} icon={BadgePercent} gradient="from-purple-500/10 to-purple-500/5 text-purple-600" />
      </div>
          ...
          <CardContent className="p-8">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-black text-main leading-tight">حالة الحماية الرقابية</h2>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mt-1">مدى صرامة القواعد المطبقة بالنظام</p>
                </div>
              </div>
              <Badge variant="outline" className="bg-emerald-50 border-emerald-200 text-emerald-600 font-black px-4 py-1">موثق ومفعل</Badge>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {checks.map((check, idx) => (
                <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-soft/40 border border-border/40 hover:bg-soft transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center shadow-sm", check.value ? "bg-emerald-100 text-emerald-600" : "bg-red-100 text-red-600")}>
                      <check.icon size={16} />
                    </div>
                    <span className="text-sm font-bold text-main">{check.title}</span>
                  </div>
                  {check.value ? <CheckCircle2 size={18} className="text-emerald-500" /> : <XCircle size={18} className="text-red-500" />}
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="rounded-[32px] border-border/50 bg-card shadow-sm overflow-hidden">
          <CardContent className="p-8">
            <div className="flex items-center gap-4 mb-8">
              ...
              <h2 className="text-xl font-black text-main leading-tight">ملخص الحركة المباشرة</h2>
            </div>
            
            <div className="space-y-4">
              <SummaryLine label="الإيراد المسجل" value={formatCurrency(metrics.revenue)} />
              <SummaryLine label="المنصرف" value={formatCurrency(metrics.expenseTotal)} />
              <div className="my-4 border-t border-dashed border-border/60" />
              <SummaryLine label="الصافي الحالي" value={formatCurrency(metrics.net)} highlight={metrics.net >= 0 ? "success" : "danger"} large />
              <div className="mt-6 p-4 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-between">
                <span className="text-xs font-bold text-blue-800">الورديات المفتوحة (درج النقود)</span>
                <span className="text-lg font-black text-blue-600">{metrics.openShifts}</span>
              </div>
            </div>
          </CardContent>
        </Card>
...
function RulesPanel({ rules, updateRule }) {
  const booleanRules = [
    { key: "enableSuspiciousDiscountAlerts", title: "تنبيهات الخصم المشبوه", description: "إصدار تنبيه للمالك عند تجاوز الخصومات لمعدلات غير طبيعية.", icon: BellRing },
    { key: "requireDiscountReason", title: "إلزام سبب الخصم", description: "لا يسمح بإضافة أي خصم على الفاتورة بدون كتابة ملاحظة توضيحية.", icon: BadgePercent },
    { key: "requireManagerApprovalAboveLimit", title: "اعتماد الخصومات العالية", description: "المطالبة بموافقة مدير/مالك عند تجاوز نسبة الخصم حد الكاشير المسموح.", icon: ShieldAlert },
    { key: "enableExpenseApproval", title: "اعتماد المصروفات", description: "لا يتم إدراج المصروفات الكبيرة بالتقارير إلا بعد مراجعتها واعتمادها.", icon: FileCheck2 },
    { key: "enableProductCostTracking", title: "تتبع تكلفة المخزون", description: "احتساب الربحية تلقائياً بناءً على متوسط تكلفة الشراء للمنتج.", icon: Calculator },
    { key: "allowNegativeInventory", title: "السماح بالمخزون السالب", description: "تمكين بيع منتجات رصيدها صفر. (لا يُنصح به لأسباب رقابية).", icon: AlertTriangle },
  ];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <Card className="rounded-[32px] border-border/50 bg-card shadow-sm lg:col-span-2 p-8">
        <h2 className="text-xl font-black text-main mb-6 flex items-center gap-3">
          <Receipt className="text-muted-foreground" size={24} /> سياسات الفواتير والخصم
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {booleanRules.map((rule) => (
            <ToggleRule key={rule.key} rule={rule} rules={rules} updateRule={updateRule} />
          ))}
        </div>
      </Card>
      ...
      <Card className="rounded-[32px] border-border/50 bg-card shadow-sm p-8">
        <h2 className="text-xl font-black text-main mb-6 flex items-center gap-3">
          <Calculator className="text-muted-foreground" size={24} /> القيم والحدود المالية
        </h2>
        <div className="space-y-6">
          <NumberField
...
function PaymentsPanel({ rules, updateRule, updatePaymentMethod, metrics }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <Card className="rounded-[32px] border-border/50 bg-card shadow-sm lg:col-span-2 p-8">
        <h2 className="text-xl font-black text-main mb-6 flex items-center gap-3">
          <Lock className="text-muted-foreground" size={24} /> ضوابط درج النقود والورديات
        </h2>
        
        <div className="space-y-4 mb-8">
          <ToggleRule 
            rule={{key: "requireOpenShiftForInvoices", title: "الربط بنقاط البيع المفتوحة", description: "يُمنع أي مستخدم من إصدار فواتير دون فتح وردية/درج نقود خاص به.", icon: Lock}} 
            rules={rules} updateRule={updateRule} layout="row"
          />
          <ToggleRule 
            rule={{key: "lockInvoicesAfterShiftClose", title: "تجميد الفواتير للورديات المغلقة", description: "تحويل فواتير الوردية المغلقة لوضع القراءة فقط (تتطلب مدير للتعديل).", icon: FileLock2}} 
            rules={rules} updateRule={updateRule} layout="row"
          />
        </div>

        <h3 className="text-sm font-black uppercase tracking-widest text-muted-foreground mb-4">قنوات الدفع المسموحة للعملاء</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {methods.map((method) => (
            <div key={method} className="flex items-center justify-between p-4 rounded-2xl bg-soft/50 border border-border/50 hover:bg-soft transition-colors">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-card border border-border/50 flex items-center justify-center text-main shadow-sm">
                  {method === "cash" ? <Banknote size={18} /> : method === "card" ? <CreditCard size={18} /> : <Wallet size={18} />}
                </div>
                <span className="font-bold text-sm text-main">{paymentLabels[method]}</span>
              </div>
              <Switch checked={Boolean(rules.enabledPaymentMethods?.[method])} onCheckedChange={(checked) => updatePaymentMethod(method, checked)} />
            </div>
          ))}
        </div>
      </Card>
      ...
      <div className="space-y-6">
        <Card className="rounded-[32px] border-border/50 bg-card shadow-sm p-8">
          <h2 className="text-xl font-black text-main mb-6 flex items-center gap-3">
            <CheckCircle2 className="text-muted-foreground" size={24} /> التفضيل الافتراضي للدفع
          </h2>
          <div className="space-y-3">
            {methods.map((method) => (
              ...
              >
                ...
                {rules.defaultPaymentMethod === method && <CheckCircle2 size={18} />}
              </button>
            ))}
          </div>
        </Card>
        ...
        <Card className="rounded-[32px] border-border/50 bg-card shadow-sm p-8">
          ...
          <div className="mt-4 border-t border-border/50 pt-4">
            <NumberField
              ...
              onChange={(value) => updateRule("maxCashDiscrepancyWithoutNote", value)}
              icon={AlertTriangle}
            />
          </div>
        </Card>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-soft/20 border-b border-border/50">
        <div>
          <h2 className="text-xl font-black text-main flex items-center gap-3">
            <Search className="text-muted-foreground" size={20} /> أرشيف الرقابة المتقدم
          </h2>
          <p className="mt-1 text-xs font-bold text-muted-foreground uppercase tracking-widest">تتبع العمليات الحساسة كالخصومات، التعديلات، وإلغاء الفواتير.</p>
        </div>
        <div className="relative w-full md:w-80 group">
          <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-accent transition-colors" size={16} />
          <Input
            value={searchTerm}
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border/50 bg-soft/30 hover:bg-soft/30">
              <TableHead className="py-4 text-xs font-black uppercase tracking-widest text-muted-foreground text-right rounded-tr-2xl">نوع الإجراء</TableHead>
              <TableHead className="py-4 text-xs font-black uppercase tracking-widest text-muted-foreground text-right">تفاصيل العملية</TableHead>
              <TableHead className="py-4 text-xs font-black uppercase tracking-widest text-muted-foreground text-right">القسم/الكيان</TableHead>
              <TableHead className="py-4 text-xs font-black uppercase tracking-widest text-muted-foreground text-right rounded-tl-2xl">طابع زمني</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody className="divide-y divide-border/50">
            {rows.map((row) => (
              <TableRow key={row.id || `${row.action}-${row.created_at}`} className="hover:bg-soft/20 transition-colors">
                <TableCell className="py-4">
                  <Badge variant="outline" className="rounded-lg px-3 py-1 bg-card text-[10px] font-bold uppercase tracking-widest">{row.action || "عملية"}</Badge>
                </TableCell>
                ...
                <TableCell className="py-4 text-xs font-bold text-muted-foreground" dir="ltr">
            ...
            {rows.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="h-64">
                  <TableEmptyState icon={FileLock2} title="لا توجد عمليات رقابية مسجلة" description="قم بتوسيع نطاق البحث أو تأكد من ربط النظام بالخادم." />
                </TableCell>
              </TableRow>
            )}
          </TableBody>
...
function MetricCard({ title, value, icon: Icon, gradient }) {
  return (
    <div className="rounded-[32px] p-6 bg-card border border-border/50 shadow-sm relative overflow-hidden group hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
      <div className={cn("absolute inset-0 bg-gradient-to-br opacity-50 pointer-events-none transition-opacity group-hover:opacity-100", gradient)} />
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{title}</p>
          <div className="h-10 w-10 rounded-xl bg-card border border-border/50 flex items-center justify-center shadow-sm text-main group-hover:scale-110 transition-transform">
            <Icon size={18} />
          </div>
        </div>
        <p className="text-3xl font-black text-main tracking-tight leading-none">{value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-start gap-4">
        <div className={cn("flex h-12 w-12 shrink-0 items-center justify-center rounded-xl shadow-sm transition-colors", isChecked ? "bg-accent/10 text-accent" : "bg-card border border-border/50 text-muted-foreground")}>
          <Icon size={20} />
        </div>
        <div>
          <p className="text-sm font-black text-main">{rule.title}</p>
          <p className="mt-1 text-xs font-bold text-muted-foreground leading-relaxed">{rule.description}</p>
        </div>
      </div>
      <div className={cn(layout === "column" && "w-full flex justify-end mt-2")}>
        <Switch checked={isChecked} onCheckedChange={(c) => updateRule(rule.key, c)} />
      </div>
    </div>
  );
}

function SummaryLine({ label, value, highlight, large }) {
  const color = highlight === "success" ? "text-emerald-600" : highlight === "danger" ? "text-red-600" : "text-main";
  return (
    <div className="flex items-center justify-between px-2">
      <span className={cn("font-bold text-muted-foreground", large ? "text-sm" : "text-xs uppercase tracking-widest")}>{label}</span>
      <span className={cn("font-black tracking-tight", color, large ? "text-2xl" : "text-lg")} dir="ltr">{value}</span>
    </div>
  );
}

function NumberField({ label, value, onChange, min = 0, max, icon: Icon }) {
  return (
