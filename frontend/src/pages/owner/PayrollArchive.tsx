...
const money = (value) => `${Number(value || 0).toLocaleString("ar-EG-u-nu-latn")} ج.م`;
...
const PayrollArchive = () => {
  ...
  const fetchArchive = useCallback(async (nextFilters = filters) => {
    try {
      setLoading(true);
      ...
      console.error("Payroll archive load error:", _err);
          ...
          <table className="w-full text-right">
            <thead>
              <tr className="border-b border-border bg-soft/30">
                <th scope="col" aria-sort="none" className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-muted">
                  الموظف
                </th>
                <th scope="col" aria-sort="none" className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-muted">
                  الشهر/السنة
                ...
                <th scope="col" aria-sort="none" className="px-6 py-5 text-center text-[11px] font-black uppercase tracking-widest text-muted">
                  الإجراءات
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {archiveRows.map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-5 text-sm font-black text-main" dir="ltr">
                    <bdi>{money(item.net_salary)}</bdi>
                  </td>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        exportService.downloadPdf(
                          `/exports/payroll/${item.id}/pdf`,
                          `payslip_${item.employee_name_snapshot}_${item.period_month}_${item.period_year}`,
                        )
                      }
                      className="h-9 rounded-lg border border-emerald-100 bg-emerald-50/50 text-emerald-700 hover:bg-emerald-100 font-bold text-[10px]"
                    >
                      تحميل PDF
                    </Button>
                  </td>
