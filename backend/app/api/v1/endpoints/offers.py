...
from app.schemas.offer import OfferCreate, OfferRead, OfferServiceRead, OfferUpdate
...
def _prepare_offer_prices(payload: OfferCreate | OfferUpdate) -> tuple:
...
def list_offers(
    ...
    rows = (
        db.query(Offer)
        .options(joinedload(Offer.offer_services).joinedload(OfferService.service))
...
def create_offer(
    ...
    original_price, discount_percentage = _prepare_offer_prices(payload)
    offer = Offer(
        name=payload.name,
